...
from Files.Tracker import Tracker,TrackerResponse



class TrackerTest(unittest.TestCase):

    # Asserts that the peer ID is exactly 20 bytes, as required by the BitTorrent protocol.
...
class TrackerResponseTest(unittest.TestCase):
    def setUp(self):
        #self.ok_response: Simulates a successful bencoded tracker response
    ...
    def test_failed_response(self):
        ...
        self.assertEqual('You failed!',response.failures)


    # checks that any valid response does not report any failure
    def test_successful_response_no_failure(self):
        response  = TrackerResponse(self.ok_response)
        self.assertIsNone(response.failures)

    def test_successful_response(self):
        response = TrackerResponse(self.ok_response)
        self.assertEqual(5500,response.completed)


    #Ensures correct leecher count from .incomplete.
    def test_successful_response_incomplete(self):
        response = TrackerResponse(self.ok_response)
        self.assertEqual(240,response.incomplete)


    #Ensures correct parsing of refresh time
    def test_successful_response_interval(self):
        response = TrackerResponse(self.ok_response)
        self.assertEqual(1800,response.interval)

    def test_successful_response_peers_string(self):
        response = TrackerResponse(self.ok_response)
